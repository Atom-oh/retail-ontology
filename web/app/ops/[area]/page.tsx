...
const META: Record<string, { ko: string; desc: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  guardrail: { ko: '가드레일',         desc: 'Bedrock Guardrail intervention 로그 (CloudWatch)', icon: ShieldCheck },
  memory:    { ko: '메모리 히스토리', desc: 'AgentCore Memory short/long-term 이벤트', icon: Brain },
  eval:      { ko: '평가 결과',        desc: '30 wow query 검색 pass rate + latency', icon: Activity },
  trace:     { ko: '도구 호출 트레이스', desc: '대화형 에이전트가 사용한 도구 호출 timeline — 최근 200개', icon: ListTree },
};
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 px-8 py-8 max-w-6xl mx-auto w-full">
        ...
        {!META[params.area] && <p className="text-sm text-ink-400">Unknown ops area: {params.area}</p>}
      </div>
    </div>
  );
}
...
function GuardrailView() {
  ...
  async function load() {
    setLoading(true); setError(null);
    try { setData(await api.opsGuardrail(minutes, 50)); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
    finally { setLoading(false); }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        {data && <span className="text-xs text-ink-400">guardrail_id: <span className="font-mono text-ink-200">{data.bedrock_guardrail_id}</span></span>}
      </div>
      {error && <ErrorBox msg={error} />}
      {data && data.events.length === 0 && (
        <p className="text-sm text-ink-400 italic px-1">매칭된 가드레일 이벤트가 없습니다 (정상 동작 중).</p>
      )}
      {data && data.events.length > 0 && (
        <ul className="space-y-1.5">
          {data.events.map((e, i) => (
            <li key={i} className="p-2.5 rounded border border-ink-700 bg-ink-800 font-mono text-xs">
              <span className="text-ink-400 mr-2">{new Date(e.timestamp).toISOString().slice(11, 19)}</span>
...
function MemoryView() {
  ...
  async function load() {
    setLoading(true); setError(null);
    try { setData(await api.opsMemory(sid.trim() || undefined, 30)); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
    finally { setLoading(false); }
  }
  useEffect(() => { load(); }, []); // initial: empty session, just shows memory_id
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <input
          value={sid} onChange={(e) => setSid(e.target.value)}
          placeholder="session_id (예: sess_abc123…)"
          className="flex-1 rounded-md border border-ink-700 bg-ink-900 text-ink-100 px-3 py-1.5 text-xs font-mono"
        />
        <button onClick={load} disabled={loading} className="px-3 py-1.5 rounded-md bg-accent-500 text-ink-950 font-semibold text-xs disabled:bg-ink-700">
          {loading ? '로딩…' : '조회'}
        </button>
      </div>
      {error && <ErrorBox msg={error} />}
      {data && (
        <Section title={`Memory: ${data.memory_id} ${data.session_id ? `· ${data.session_id}` : ''}`}>
          {!data.session_id && (
            <p className="text-xs text-ink-400 italic">session_id를 입력하면 short-term 이벤트(role/text/actor)가 시간 순으로 표시됩니다.</p>
          )}
...
function EvalView() {
  ...
  async function load(run = false) {
    setLoading(true); setError(null);
    try { setData(await api.opsEval(run)); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
    finally { setLoading(false); }
  }
  useEffect(() => { load(false); }, []);
  if (loading && !data) return <p className="text-sm text-ink-400">로딩 중… (라이브 실행은 30s+ 소요)</p>;
  if (error) return <ErrorBox msg={error} />;
  if (!data) return null;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-ink-400">cached at {new Date(data.cached_at_unix * 1000).toLocaleTimeString()}</span>
      </div>
...
function TraceView() {
  ...
  async function load() {
    setLoading(true); setError(null);
    try { setData(await api.opsTrace(100)); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
    finally { setLoading(false); }
  }
  useEffect(() => { load(); }, []);
        ...
        {filtered.map((e, i) => (
          <li key={i} className="p-3 rounded border border-ink-700 bg-ink-800">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="text-[10px] font-mono text-ink-500">@ {new Date(e.ts * 1000).toLocaleTimeString()}</span>
...
function ErrorBox({ msg }: { msg: string }) {
  return <div className="p-3 rounded-md bg-red-500/10 text-red-300 border border-red-500/30 text-sm">{msg}</div>;
}
function RefreshButton({ onClick, loading }: { onClick: () => void; loading: boolean }) {
  return (
    <button onClick={onClick} disabled={loading} className="px-3 py-1.5 rounded-md border border-ink-700 text-ink-200 text-xs hover:bg-ink-800 disabled:opacity-50">
