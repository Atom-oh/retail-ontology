...
// Sidebar footer auth widget — fetches /api/auth/whoami to determine if a
// user is logged in and shows their email plus a logout button. Used by
// Sidebar.tsx in the bottom slot. Self-contained so the parent component
// doesn't need to thread auth state through props.
...
export function SidebarAuth() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/auth/whoami`, { credentials: 'include' })
      .then(async (r) => {
        // 401 returns JSON `{authenticated: false}` — still parse it.
